...
LastEditTime: 2022-12-21 14:55:14
Description: 获取点击数据（正负样本比例1:4)并且处理，用于训练rank模型

conda activate py38
nohup python -u click_data4bert_classify_cv_vec.py > click_data4bert_classify_cv_vec.out 2>&1 &
tail -f click_data4bert_classify_cv_vec.out
ps -ef | grep click_data4bert_classify_cv_vec.py
kill -9 `ps -aef | grep 'click_data4bert_classify_cv_vec.py' | grep -v grep | awk '{print $2}'`
...
class ClickData(object):
    ...
    def get_click_datas4bert(self, day):
        ...
        for info in presto_cursor.fetchall():
            ...
            if query not in click_datas:
                query_vec, docs_vec, time_consume = get_pic_vec(query=query,dim=128)
                click_datas[query] = {
                    'query': query, "query_vec":query_vec, 'positive_ids_rele': [], 'negative_ids_rele': []}
                # pos_neg_num_dict[query] = {"pos_num": 0, "neg_num": 0}
            
            query_vec = click_datas[query]["query_vec"] # 获取query_vec
            # 正样本
            if relevance > 0:
                if relevance > 1:  # 有交互行为的数据是相关性>1 的正样本
                    
                    click_datas[query]['positive_ids_rele'].append(
                        (doc_id, relevance, favorite_num, like_num, comment_num, score, interval_days, wiki, obj_type, query_vec))
    ...
    def get_corpus(self, obj_ids_list):
        ...
        '''
        from data_utils.Thread.dataProcessFromDb_128 import dataProcessFromDb
            ...
            if len(click_data['positive_ids_rele']) > 0 and len(click_data['negative_ids_rele']) > 0:
                ...
                for pos_id_rele_data in click_data['positive_ids_rele']:
                    if pos_id_rele_data[0] in corpus_obj_ids_set:
                        ...
                        # pos_id_rele_data = (doc_id, relevance, favorite_num, like_num, comment_num, score, interval_days, wiki, obj_type, query_vec))
                        
                        # for d in pos_id_rele_data[2:]:
                        #     data4train.append(d)
                        data4train = list(np.concatenate((data4train, pos_id_rele_data[2:]),axis=0))
                        data4train.append(corpus[pos_id_rele_data[0]]["cv_vec"])
                        ...
                        '''if query in title[:max_len] or query in remark[:max_len]: # 这个case 会把那些同义词的结果都变成负样本
                            pos_cnt += 1
                            label = str(pos_id_rele_data[1])
                            data4train = [query, title, remark, str(label), title_tf_num, remark_tf_num]
                ...
                for neg_id_rele_data in click_data['negative_ids_rele']:
                    if neg_id_rele_data[0] in corpus_obj_ids_set:
                        if neg_cnt < pos_cnt*pos_neg_ration:
                            ...
                            #     data4train.append(d)
                            data4train.append(corpus[neg_id_rele_data[0]]["cv_vec"])
                            samples.append(data4train)
...
def get_click_data(a=1, b=2, c=1):
    ...
    # for day in range(1, 5, 1):
        ...
        pkl_file = os.path.join('/data', folder_name, 'topk_opt/rank_data_classify_distil_cv_vec/data_'+get_before_day(day)+'.pkl')
        npy_classify_file = os.path.join('/data', folder_name, 'topk_opt/rank_data_classify_eval_distil_cv_vec/eval_classify_'+get_before_day(day)+'.npy')
        ...
        log.logger.info("数据读取完成 rank_data_classify_distil_cv vec "+ str(day))
        send_msg("数据读取完成 rank_data_classify_distil_cv vec "+ str(day))
    log.logger.info("程序执行完毕")
    send_msg("pkl文件数据 rank_data_classify_distil_cv vec 生成完成")


if __name__ == '__main__':
    get_click_data()
    # get_click_data(23,20,-1)
    # get_click_data(20,17,-1)
    # get_click_data(17,14,-1)
    # get_click_data(14,11,-1)
    # get_click_data(11,8,-1)
    # get_click_data(8,5,-1)
    # get_click_data(5,3,-1)
    # get_click_data(3,0,-1)

    # get_click_data(5,6,1)
