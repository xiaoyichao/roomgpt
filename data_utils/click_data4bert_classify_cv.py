...
LastEditTime: 2022-12-20 13:46:51
Description: 获取点击数据（正负样本比例1:4)并且处理，用于训练rank模型

conda activate py38
nohup python -u click_data4bert_classify_cv.py > click_data4bert_classify_cv.out 2>&1 &
tail -f click_data4bert_classify_cv.out
ps -ef | grep click_data4bert_classify_cv.py
kill -9 `ps -aef | grep 'click_data4bert_classify_cv.py' | grep -v grep | awk '{print $2}'`
    ...
    def get_click_datas4bert(self, day):
        ...
        for info in presto_cursor.fetchall():
            ...
            if query not in click_datas:
                query_vec, docs_vec, time_consume = get_pic_vec(query=query)
                click_datas[query] = {
...
def get_click_data(a=1, b=2, c=1):
    ...
    # for day in range(1, 5, 1):
        ...
        pkl_file = os.path.join('/data', folder_name, 'topk_opt/rank_data_classify_distil_cv/data_'+get_before_day(day)+'.pkl')
        npy_classify_file = os.path.join('/data', folder_name, 'topk_opt/rank_data_classify_eval_distil_cv/eval_classify_'+get_before_day(day)+'.npy')
        ...
        # send_msg("数据读取完成"+ str(day))
    log.logger.info("程序执行完毕")
    send_msg("pkl文件数据 rank_data_classify_distil_cv 生成完成")


if __name__ == '__main__':
    get_click_data(3,5,1)
