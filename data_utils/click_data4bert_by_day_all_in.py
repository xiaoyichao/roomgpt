...
LastEditTime: 2022-08-02 16:06:07
...
class ClickData(object):
    ...
    def get_click_datas4bert(self, day):
        ...
        for info in presto_cursor.fetchall():
            ...
            if query not in hashQueryObjIdsTotalNum:
                hashQueryObjIdsTotalNum[query] = {}

            hashQueryObjIdsTotalNum[query][obj_id] = total_num
        ...
        return click_datas, corpus_obj_ids_set, all_use_obj_id, hashQueryObjIdsTotalNum
    ...
    def get_samples(self, click_datas, corpus, hashQueryObjIdsTotalNum, max_relevance=3, pos_neg_ration=4, max_len=32):
        '''
        Author: xiaoyichao
        param {*}
        ...
        for unique_str, click_data in click_datas.items():
            ...
            if len(click_data['positive_ids_rele']) > 0 and len(click_data['negative_ids_rele']) > 0:
                ...
                for pos_id_rele_data in click_data['positive_ids_rele']:
                    ...
                    if objId in corpus_obj_ids_set:
                        ...
                        if query in title[:max_len] or query in remark[:max_len] or \
                            ...
                            trueObjIdsSet.add(objId)
                            pos_cnt_tmp += 1
                        else:
                            negative_ids_set.add(objId)
                            neg_cnt_tmp += 1
                ...
                # 在负样本里出现过的正样本
                int_negative_ids_set = negative_ids_set & trueObjIdsSet

                truePositiveDatasSample = []
                truePositiveDatasProSample = []
                ...
                for pos_id_rele_data in click_data['positive_ids_rele']:
                    ...
                    if objId in corpus_obj_ids_set:
                        ...
                        if query in title[:max_len] or query in remark[:max_len] or \
                            ...
                            if objId in int_negative_ids_set :
                                truePositiveDatasSample.append(data4train)
                                truePositiveDatasProSample.append(hashQueryObjIdsTotalNum[query][objId])
                            ...
                            # 难分的负样本
                            hardNegativeDatas.append(data4train)

                            samples.append(data4train)

                # 负样本 里 在 库中 出现
                int_obj_ids = corpus_obj_ids_set & negative_ids_set

                # 构建负样本
                trueNegativeDatas = []

                if len(int_obj_ids) > 0:
                    # 先获取一遍 负样本
                    ...
                    for neg_id_rele_data in click_data['negative_ids_rele']:
                        ...
                        if objId in corpus_obj_ids_set:
                            if neg_cnt < pos_cnt * pos_neg_ration:
                                ...
                                trueNegativeDatas.append(data4train)
                            else:
                                break
                    ...
                    # 合并  普通负样本 和 难分的负样本 数据
                    allMergeNegativeDatas = []
                    allMergeNegativeDatas.extend(hardNegativeDatas)
                    allMergeNegativeDatas.extend(trueNegativeDatas)

                    # 被越多用户 点过的内容 越不容易被采样为 负样本
                    if len(truePositiveDatasProSample) > 0:
                        truePositiveDatasProNp = np.array(truePositiveDatasProSample) * -1
                        truePositiveDatasProNpStd = np.exp(truePositiveDatasProNp - np.max(truePositiveDatasProNp)) / np.sum(np.exp(truePositiveDatasProNp - np.max(truePositiveDatasProNp)))

                    # 1个正样本 采样 + 4个 负样本 采样
                    if len(allMergeNegativeDatas) > 0:
                        while True:
                            if neg_cnt < pos_cnt * pos_neg_ration:
                                if len(truePositiveDatasProSample) > 0:
                                    truePositiveData = np.random.choice(truePositiveDatasSample,
                                                                        p=truePositiveDatasProNpStd.ravel(),
                                                                        size=1, replace=False)

                                    samples.append(truePositiveData)

                                    neg_cnt = neg_cnt + 1
                                    if neg_cnt >= pos_cnt * pos_neg_ration:
                                        break

                                random.shuffle(allMergeNegativeDatas)

                                count = 0
                                for mergeNegativeData in allMergeNegativeDatas:
                                    count = count + 1
                                    if count > pos_neg_ration:
                                        break

                                    samples.append(mergeNegativeData)
...
def get_click_data():
    ...
    clickdata = ClickData()
    for day in range(68, 1, -1):
        click_datas, corpus_obj_ids_set, all_use_obj_id, hashQueryObjIdsTotalNum = clickdata.get_click_datas4bert(day=day)
        ...
        samples = clickdata.get_samples(click_datas=click_datas, corpus=corpus_sum, max_relevance=max_relevance, pos_neg_ration=POS_NEG_RATION, max_len=max_len, hashQueryObjIdsTotalNum = hashQueryObjIdsTotalNum)
        log.logger.info("len(samples): " + str(len(samples)))

        pkl_file = os.path.join('/data/search_opt_model/topk_opt/rank_data_all_in/data_'+get_before_day(day)+'.pkl')
