...
from data_utils.data_process4bert_pretraining import getProcessBlank, getProcessArticleInfo, getProcessNoteInfo, getProcessGuide
...
class dataProcessFromDbInPretrain(Process):
    '''
    线程类，注意需要继承线程类Thread
    '''
    # 标记线程类
    TYPE = "dataProcessFromDbInPretrain"
    ...
    def __init__(self, thread_id, objIdQueue, data_queue, objNum, content_type, objType, globalTaskNum):
        ...
        """
        super(dataProcessFromDbInPretrain,self).__init__()
            ...
            while True:
                ...
                if len(objIds) > 0:
                    if self.content_type == 1:
                        objid_title_remark_dict = getProcessBlank(objIds, init_resource_cls_exterior=init_resource_cls)
                        self.data_queue.put(objid_title_remark_dict)
                    elif self.content_type == 2:
                        objid_title_remark_dict = getProcessNoteInfo(objIds, use_presto=True, init_resource_cls_exterior=init_resource_cls)
                        self.data_queue.put(objid_title_remark_dict)
                    elif self.content_type == 3:
                        objid_title_remark_dict = getProcessArticleInfo(objIds, init_resource_cls_exterior=init_resource_cls)
                        self.data_queue.put(objid_title_remark_dict)
                    elif self.content_type == 4:
                        objid_title_remark_dict = getProcessGuide(objIds)
                        self.data_queue.put(objid_title_remark_dict)
