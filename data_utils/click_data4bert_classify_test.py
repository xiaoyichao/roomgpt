...
LastEditTime: 2022-12-21 18:12:26
Description: 获取点击数据（正负样本比例1:4)并且处理，用于训练rank模型

conda activate py38
nohup python -u click_data4bert_classify_test.py > click_data4bert_classify_test.out 2>&1 &
tail -f click_data4bert_classify_test.out
ps -ef | grep click_data4bert_classify_test.py
kill -9 `ps -aef | grep 'click_data4bert_classify_test.py' | grep -v grep | awk '{print $2}'`
'''
...
def get_click_data(a=1, b=2, c=1):
    ...
    # for day in range(1, 5, 1):
        ...
        pkl_file = os.path.join('/data', folder_name, 'topk_opt/rank_data_classify_test/data_'+get_before_day(day)+'.pkl')
        npy_classify_file = os.path.join('/data', folder_name, 'topk_opt/rank_data_classify_eval_test/eval_classify_'+get_before_day(day)+'.npy')
    ...
    send_msg("pkl文件数据 rank_data_classify_distil_test 生成完成")


if __name__ == '__main__':
    get_click_data(21,18,-1)
    get_click_data(18,16,-1)
