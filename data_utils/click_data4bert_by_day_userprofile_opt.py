...
featureEncodeJsonFile = "/data/search_opt_model/topk_opt/user_profile_rerank_seq_opt/feature_encode.json"
...
class ClickData(object):
    ...
    def get_click_datas4bert(self, day):
        ...
        sql = '''with a as (select
                    ...
                    where  day  between #{date-%s,yyyyMMdd} and #{date-%s,yyyyMMdd} and query is not  null and obj_id is not null and relevance is not null and is_valid is not null and (relevance > 1 or (is_valid = 1 and relevance = 1)) and position <= 40  group by query, search_request_id, uid),
b as (select count(*) as num, query from a group by query),
c as (select * from b where num > 10),
d as (select * from c join a on a.query = c.query),
e as (select count(*) as uid_num, uid from d group by uid),
f as (select uid_num, uid from e where uid_num > 3),
g as (select * from f join a on f.uid = a.uid),
h as (select count(query) as num, query from g group by query ),
i as (select * from h where num > 10),
j as (select
                        z.search_request_id
                    from oss.hhz.dws_search_user_personalization_rerank_daily_report as z join i on i.query = z.query
                    where  z.day  between #{date-%s,yyyyMMdd} and #{date-%s,yyyyMMdd} and z.query is not  null and z.obj_id is not null and z.relevance is not null and z.is_valid is not null and  (z.relevance > 1 or (z.is_valid = 1 and z.relevance = 1)) and z.position <= 40 group by z.search_request_id ),
k as (select  
    query, 
    obj_id, 
    ln(1+max(zz.favorite_num)) favorite_num, 
    ln(1+max(zz.like_num)) like_num, 
    ln(1+max(zz.comment_num)) comment_num, 
    max(zz.score) score, 
    max(zz.add_day) add_day, 
    max(zz.wiki) wiki, 
    max(zz.user_identity_type) user_identity_type,
    max(zz.obj_type) obj_type, 
    min(zz.position) position, 
    max(zz.gender) gender,
    max(zz.age) age, 
    max(zz.is_valid) is_valid,
    max(zz.relevance) relevance, 
    day, 
    uid, 
    max(zz.search_request_id) search_request_id,
    max(zz.topic_id) topic_id, 
    max(zz.topic_cate) topic_cate, 
    max(zz.cate) cate, 
    max(zz.subcate) subcate, 
    max(zz.author_uid) author_uid,
    max(zz.author_type) author_type, 
    max(zz.author_city) author_city, 
    max(zz.author_gender) author_gender, 
    max(zz.user_city) user_city, 
    max(zz.decoration_status) decoration_status,
    ln(1+max(zz.item_like_7day)) item_like_7day, 
    ln(1+max(zz.item_comment_7day)) item_comment_7day,
  ln(1+max(zz.item_favorite_7day)) item_favorite_7day, 
  ln(1+max(zz.item_click_7day)) item_click_7day, 
  ln(1+max(zz.item_exp_7day)) item_exp_7day, 
  ln(1+max(zz.item_likek_30day)) item_likek_30day, 
  ln(1+max(zz.item_comment_30day)) item_comment_30day, 
  ln(1+max(zz.item_favorite_30day)) item_favorite_30day, 
  ln(1+max(zz.item_click_30day)) item_click_30day,
  ln(1+max(zz.item_exp_30day)) item_exp_30day, 
  ln(1+max(zz.author_becomment)) author_becomment, 
  ln(1+max(zz.author_befollow)) author_befollow, 
  ln(1+max(zz.author_belike)) author_belike, 
  ln(1+max(zz.author_befavorite)) author_befavorite, 
  ln(1+max(zz.action_count_like)) action_count_like,
  ln(1+max(zz.action_count_comment)) action_count_comment, 
  ln(1+max(zz.action_count_favor)) action_count_favor, 
  ln(1+max(zz.action_count_all_active)) action_count_all_active,
  max(top_score) top_score,
  'a' flag
...
zz.day  between #{date-%s,yyyyMMdd} and #{date-%s,yyyyMMdd} and zz.query is not  null and zz.obj_id is not null and zz.relevance is not null and zz.is_valid is not null  and zz.position <= 40 group by query, obj_id, day, uid),
l as (
    select 
...
select 
                        ...
                        top_score as "精排得分"
 from k join l on k.flag=l.flag order by uid desc, query desc, position asc

''' % (dayTmp + 1, day + 1, day, day, day, day)
        ...
        for info in fetchDatas:
            ...
            if uid not in hashUidQueryObjScore:
                hashUidQueryObjScore[uid] = {}

            if query not in hashUidQueryObjScore[uid]:
                hashUidQueryObjScore[uid][query] = {}

            hashUidQueryObjScore[uid][query][doc_id] = top_score
            '''数据预处理'''
            ...
            unique_str = str(query) + "_" + str(uid)

            if unique_str not in click_datas:
                click_datas[unique_str] = {
                    'query': query, 'uid' : uid, 'list': [], 'is_have_positive' : 0}
            ...
            if relevance > 1:
                lable = 2
            elif relevance == 1 and is_valid == 1:
                lable = 1
            else:
                lable = 0
        ...
        for unique_str, click_data in  click_datas.items():
            if click_data["is_have_positive"] == 1:
                click_data_formats[unique_str] = {
                    'query': click_data["query"], 'uid': click_data["uid"],  'list': click_data["list"]
                }
        ...
        with a as (select max(day) as max_day, obj_id from (select
                       obj_id,day 
                    from oss.hhz.dws_search_user_personalization_rerank_daily_report
                    where  day  between #{date-%s,yyyyMMdd} and #{date-%s,yyyyMMdd} and query is not  null and obj_id is not null and relevance is not null and is_valid is not null  and position <= 40 and (is_valid = 1 or relevance > 1)) group by obj_id),
b as (select
                    ...
                    where  aa.day  between #{date-%s,yyyyMMdd} and #{date-%s,yyyyMMdd} and aa.query is not  null and aa.obj_id is not null and aa.relevance is not null and aa.is_valid is not null and aa.position <= 40 and (is_valid = 1 or relevance > 1))
select * from b
        ...
        select uid, obj_id_list from n''' % (dayTmp + 1, timeday + 1)
        ...
        for unique_str, click_data_format in  click_data_formats.items():
            query = click_data_format["query"]
            for item in click_data_format["list"]:
                uid = str(item[13])
                docId = str(item[0])

                docIdIndex = featureEncodeHashInfo["obj_id"]["hash_info"][str(item[0])]
                ...
                if uid in hashUserValidClickSeq:
                    ...
                    if docIdIndex in doc_id_seq:
                        isDeleteValidClickSeq = 1
                        delIndex = doc_id_seq.index(docIdIndex)
        ...
        return click_data_formats, corpus_obj_ids_set, hashUidQueryObjScore
    ...
    def get_samples(self, click_datas, corpus, hashUidQueryObjScore):
        '''
        Author: xiaoyichao
        param {*}
        ...
        for unique_str, click_data in click_datas.items():
            ...
            for item in click_data["list"]:
                if item[0] in corpus_obj_ids_set:
                    query = str(click_data["query"])
                    docId = str(item[0])
                    uid = str(click_data["uid"])

                    topScore = 0
                    if uid in hashUidQueryObjScore:
                        if query in hashUidQueryObjScore[uid]:
                            if docId in hashUidQueryObjScore[uid][query]:
                                topScore = hashUidQueryObjScore[uid][query][docId]
                    ...
                    data4train = [topScore, item[0], item[1], query, title, remark, title_tf_num, remark_tf_num]
                    for d in item[2:]:
                        data4train.append(d)
...
def get_click_data():
    ...
    for day in range(1, 2, 1):
        click_datas, corpus_obj_ids_set, hashUidQueryObjScore = clickdata.get_click_datas4bert(day=day)
        ...
        click_format_datas = clickdata.get_samples(click_datas = click_datas, corpus=corpus_sum, hashUidQueryObjScore=hashUidQueryObjScore)
        ...
        pkl_file = os.path.join('/data/search_opt_model/topk_opt/user_profile_rerank_seq_opt/rank_data_userprofile/data_'+get_before_day(day)+'.pkl')
