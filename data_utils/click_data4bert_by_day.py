...
LastEditTime: 2022-08-02 16:07:56
Description: 获取点击数据并且处理，用于训练rank模型

conda activate py38
nohup python -u click_data4bert_by_day.py > click_data4bert_by_day.out 2>&1 &
tail -f click_data4bert_by_day.out
ps -ef | grep click_data4bert_by_day.py
'''
...
def get_click_data():
    ...
    for day in range(1, 2, 1):
        click_datas, corpus_obj_ids_set = clickdata.get_click_datas4bert(day=day)
        ...
        pkl_file = os.path.join('/data/search_opt_model/topk_opt/rank_data_more/data_'+get_before_day(day)+'.pkl')
    ...
    send_msg("pkl文件数据4bert_rank生成完成")
