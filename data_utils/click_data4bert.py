...
LastEditTime: 2022-08-02 16:08:08
...
def get_tf_web(query, query_word_list, sentence, have_jiebad=False, max_len=128, isGetUnique = False):
    '''
    Author: xiaoyichao
    param {*}
    ...
    if have_jiebad:
        ...
        sentence = sentence  # max_len个字
    else:
        sentence_word_list = JiebaHHZ.SplitWords(sentence[:max_len], isGetUnique = isGetUnique)
...
class ClickData(object):
    ...
    def get_click_datas4bert(self, before_day, end_day):
        ...
        for info in presto_cursor.fetchall():
            ...
            # obj_type = info[9] # obj_type_dict = {"文章": 1, "单图": 2, "多图": 3, "整屋": 4, "视频": 5, "纯文字": 6, "未知":0}  这个目前没使用,是日志里的标准
            ...
            if relevance > 0:
                if relevance > 1:  # 有交互行为的数据是相关性>1 的正样本
                    ...
                    if relevance == 1:
                        relevance = is_valid  # 有效点击是相关性为1的正样本
                        click_datas[query]['positive_ids_rele'].add(
                            (doc_id, relevance, favorite_num, like_num, comment_num, score, interval_days, wiki, obj_type))
                        corpus_obj_ids_set.add(doc_id)
                    else:
                        relevance = 0  # 点击了但是是无效点击是负样本
...
if __name__ == '__main__':
    ...
    # samples = clickData.read_samples_pkl("/data/xiaoyichao/projects/bert_regress_v2/data/rank_data_more/dev.pkl")
    # print(len(samples))
    ...
    if get_host_ip() == "172.16.10.10":
        ...
        NUM_DEV_QUERIES = 1e5
    clickdata = ClickData()
    train_click_datas, train_corpus_obj_ids_set = clickdata.get_click_datas4bert(before_day=1, end_day=1)
