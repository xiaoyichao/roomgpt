...
class ObjVecId2Index(object):
    ...
    def getAllObjId2Index(cls, init_resource_cls):
        ...
        Returns: list
        """
        sql = '''select id, obj_id from hhz_search.obj_vec_id2index where status = 1'''
        ...
        for info in content_cursor:
            objId2Index[info[1]] = int(info[0])

        return objId2Index

    @classmethod
    def getAllOIndex2ObjId(cls, init_resource_cls):
        """
        获取全量 obj id 和 id 数据
        Returns: list
        """
    ...
    def insertDatas(cls, init_resource_cls, datas):
        if len(datas) > 0:
            ...
            sql = 'insert IGNORE into hhz_search.obj_vec_id2index (`obj_id`, `status`) VALUES ' + insertStr
...
if __name__ == '__main__':
    ...
    print(ObjVecId2Index.insertDatas(InitResourceCls, ["aaaa", "bbbb"]))
