...
from presto_con import constants
import presto_con.exceptions
import presto_con.client
import presto_con.presto_logging
import presto_con.redirect
from presto_con.transaction import Transaction, IsolationLevel, NO_TRANSACTION
...
logger = presto_con.presto_logging.get_logger(__name__)
...
class Connection(object):
    ...
    def __init__(
        ...
        redirect_handler=presto_con.redirect.GatewayRedirectHandler(),
        ...
        self._http_session = presto_con.client.PrestoRequest.http.Session()
    ...
    def _create_request(self):
        return presto_con.client.PrestoRequest(
            self.host,
            self.port,
            self.user,
            self.source,
...
class Cursor(object):
    ...
    def setinputsizes(self, sizes):
        raise presto_con.exceptions.NotSupportedError

    def setoutputsize(self, size, column):
        raise presto_con.exceptions.NotSupportedError

    def execute(self, operation, params=None):
        self._query = presto_con.client.PrestoQuery(self._request, sql=operation)
    ...
    def fetchone(self):
        ...
        try:
            ...
            return None
        except presto_con.exceptions.HttpError as err:
            raise presto_con.exceptions.OperationalError(str(err))
    ...
    def cancel(self):
        if self._query is None:
            raise presto_con.exceptions.OperationalError("Cancel query failed; no running query")
        self._query.cancel()
