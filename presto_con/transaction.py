...
class Transaction(object):
    ...
    def begin(self):
        ...
        if not response.ok:
            raise presto_con.exceptions.DatabaseError(
    ...
    def commit(self):
        query = presto_con.client.PrestoQuery(self._request, COMMIT)
        try:
            list(query.execute())
    ...
    def rollback(self):
        query = presto_con.client.PrestoQuery(self._request, ROLLBACK)
        try:
            list(query.execute())
        except Exception as err:
            raise presto_con.exceptions.DatabaseError(
