...
LastEditTime: 2021-10-26 10:27:04
Description: 
'''
...
class FilterCoarseGraindRank(object):
    '''
    @Author: xiaoyichao
    @param {*}
    ...
    @classmethod
    def ranking_all4representation_based(cls, query, params_collect_cls, esRecallFaissMapIdsFilter, hashEsDataInfos, init_request_global_var, esRecallFaissMapIdsList, queryVec):
        '''
        Author: xiaoyichao
        param {*}
        Description: 基于表示的模型，根据相似度和信息量的乘积进行排序
        '''        
        start_time = time.time()
        # 构造模型推断的数据格式
        # query_doc_list = DataProcess.buildSortedModelData(esRecallFaissMapIdsFilter, query, hashEsDataInfos)
        ...
        # similars_with_information_np = cosine_similars
    ...
    # @classmethod
    # def get_coarse_graind_rank_topk(cls, similars):
    #     """
    #     获取topK
    #     Args:
    #         similars: 粗排模型的分数
    ...
    @classmethod
    def filter_data_not_TopK(cls, need_save_index, esRecallObjIds):
        # print("need_save_index", need_save_index)
        # print("esRecallObjIds", esRecallObjIds)
        esRecallObjIds = np.array(esRecallObjIds)
        return esRecallObjIds[need_save_index]