...
@Desciption : 获取 主服务地区 total article 信息的线程
'''
import threading
import os
...
class GetAreaDesignerTotalArticle(threading.Thread):
    '''
    线程类，注意需要继承线程类Thread
    '''
    # 标记线程类
    TYPE = "GetAreaDesignerTotalArticle"
    ...
    def recall(self):
        ...
        try:
            starttime = time.time()
            areaDesignerNotes = self.esRecallCls.getAreaDesignerTotalArticles(
                self.es_recall_params_cls, self.init_resource_cls, self.init_request_global_var)
            endtime = time.time()
            log.logger.info("GetAreaDesignerTotalArticle 运行时间：{:.10f} s".format(
