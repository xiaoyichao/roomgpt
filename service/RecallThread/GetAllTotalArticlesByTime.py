...
Date: 2021-06-02 10:57:50
LastEditTime: 2021-06-10 16:16:20
Description: 
'''
# coding=UTF-8
'''
...
@Desciption : 获取全量非差 整屋，文章，指南 信息的线程
'''
import threading
import os
...
class GetAllTotalArticlesByTime(threading.Thread):
    '''
    线程类，注意需要继承线程类Thread
    '''
    # 标记线程类
    TYPE = "GetAllTotalArticlesByTime"
    ...
    def recall(self):
        ...
        try:
            starttime = time.time()
            allTotalArticles = self.esRecallCls.getAllTotalArticleInfosByTime(
                self.es_recall_params_cls, self.init_resource_cls, self.init_request_global_var)

            endtime = time.time()
            log.logger.info("GetAllTotalArticlesByTime 运行时间：{:.10f} s".format(
