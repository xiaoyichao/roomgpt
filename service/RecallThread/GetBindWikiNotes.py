...
class GetBindWikiNotes(threading.Thread):
    '''
    线程类，注意需要继承线程类Thread
    '''
    # 标记线程类
    TYPE = "GetBindWikiNotes"
    ...
    def recall(self):
        ...
        try:
            starttime = time.time()
            bindWikiNotes = self.esRecallCls.getBindWikiNotes(
                self.es_recall_params_cls, self.init_resource_cls, self.init_request_global_var)
            endtime = time.time()

            log.logger.info("GetBindWikiNotes 运行时间：{:.10f} s".format(
