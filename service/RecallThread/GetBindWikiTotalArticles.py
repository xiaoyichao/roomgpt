...
LastEditTime: 2021-08-09 15:54:34
Description: 
'''
# coding=UTF-8
'''
...
class GetBindWikiTotalArticles(threading.Thread):
    '''
        线程类，注意需要继承线程类Thread
        '''
    # 标记线程类
    TYPE = "GetBindWikiTotalArticles"
    ...
    def recall(self):
        ...
        try:
            starttime = time.time()
            BindWikiTotalArticles = self.esRecallCls.getBindWikiTotalArticles(self.es_recall_params_cls, self.init_resource_cls, self.init_request_global_var)
            endtime = time.time()
            log.logger.info("GetBindWikiTotalArticles 运行时间：{:.10f} s".format(
            ...
            # print("BindWikiTotalArticles:", BindWikiTotalArticles)
            self.queue.put(
                {
                    "type" : self.TYPE,
                    "data" : BindWikiTotalArticles
                }
            )
...
if __name__ == '__main__':
    ...
    dataQueue = Queue()
    esCls = EsRecall()
    cls = GetBindWikiTotalArticles(GetBindWikiTotalArticles.TYPE,
                                          dataQueue,  esCls)
    cls.start()
    cls.join()
