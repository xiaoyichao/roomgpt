...
Date: 2021-06-10 15:59:52
LastEditTime: 2021-06-10 16:00:18
Description: 
'''
# coding=UTF-8
'''
...
class GetAllTotalArticles(threading.Thread):
    '''
    线程类，注意需要继承线程类Thread
    '''
    # 标记线程类
    TYPE = "AllTotalArticles"
    ...
    def recall(self):
        ...
        try:
            starttime = time.time()
            allTotalArticles = self.esRecallCls.getAllTotalArticles(self.es_recall_params_cls, self.init_resource_cls, self.init_request_global_var)
            endtime = time.time()
            log.logger.info("getAllTotalArticles 运行时间：{:.10f} s".format(
