...
@Desciption : 获取 wiki 双高池 note 信息的线程
'''
import threading
import os
...
class GetWiki2HighNotes(threading.Thread):
    '''
    线程类，注意需要继承线程类Thread
    '''
    # 标记线程类
    TYPE = "GetWiki2HighNotes"
    ...
    def recall(self):
        ...
        try:
            starttime = time.time()
            wiki2HighNotes = self.esRecallCls.getWiki2HighNotes(
                self.es_recall_params_cls, self.init_resource_cls, self.init_request_global_var)
            endtime = time.time()

            log.logger.info("getWiki2HighNotes 运行时间：{:.10f} s".format(
