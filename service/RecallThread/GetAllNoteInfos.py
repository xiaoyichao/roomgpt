...
class GetAllNoteInfos(threading.Thread):
    '''
    线程类，注意需要继承线程类Thread
    '''
    # 标记线程类
    ...
    def recall(self):
        ...
        try:
            starttime = time.time()
            allNoteInfos = self.esRecallCls.getAllNoteInfos(
                self.es_recall_params_cls, self.init_resource_cls, self.init_request_global_var)
            endtime = time.time()
            log.logger.info("getAllNoteInfos 运行时间：{:.10f} s".format(
